...
# Group : Dhaval, Aditi, Harmeet, Swapnil
#	v0.2
#	
trap ctrl_c INT
function ctrl_c() {
...
echo " *********        /                /     **********          /                /          "
echo
echo "    Author : Swapnil Mengi"
echo
echo "[>] Please select the service:"
echo "[1] vulnerbilty scan on a network"
echo "[2] check where you're email ID has been compromised"
read option
if [ $option -eq "2" ]; then
echo "Enter your email address"
read email
curl https://haveibeenpwned.com/api/v2/breachedaccount/$email | sed -e 's/[{}]/''/g' | awk -v k="html" '{n=split($0,a,","); for (i=1; i<=n; i++) print a[i]}'
...
function usage {
	echo
	echo -ne "Usage: openVAS-scanner.sh <host>"
	echo
...
function omp_cmd {
	cmd="omp --username $USER --password \"$PASS\" $@"
...
loadingText="[+] Authenticating with OpenVAS" 
...
if [ -z "$target_id" ]; then
	...
	out=$(omp --username "$USER" --password "$PASS" --xml="<create_target><name>$TARGET</name><hosts>$TARGET</hosts></create_target>")
